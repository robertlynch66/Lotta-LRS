...
aggregate (age_1945~lotta,FUN = mean, data=data)
...
p_full <- p_full %>% filter (age_1945 >17 & age_1945 <46)
# center age
...
summary(model)
model <- glm(reproduced ~ lotta+ age_1945, family="binomial", data=data)
summary(model)
#  
#  Model 2a
#Already ran it
...
# add age squared to model
data <- data[data$age_1945>0,]
data$age_sq <- data$age_1945*data$age_1945
model2 <-coxph(c ~ lotta + birth_cat+age_1945+age_sq+education+agriculture+repro_within_2_years+lotta:age_1945+
                 lotta:age_sq+frailty(birthplaceid_seq),data=data)
